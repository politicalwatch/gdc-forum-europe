 ...
 */
export default function useGdprAcceptQueryParam(
  paramName: string,
  setGdprAccept: (gdprAccept: boolean) => unknown
) {
  ...
  useEffect(() => {
    if ('URLSearchParams' in window) {
      ...
      const gdprAccept = params.get(paramName) === 'true';
      if (gdprAccept) {
        setGdprAccept(gdprAccept); // Convert string to boolean
  ...
  }, [setGdprAccept, router.pathname, paramName]);
}
